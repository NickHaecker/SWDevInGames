...
foreach (object simpleOb in simpleObjectList)
{
    ...
    Type type = simpleOb.GetType();
    if (type == typeof(SimpleCircle))
    {
        SimpleCircle circle = (SimpleCircle)simpleOb;
        calculatedArea = circle.Radius * circle.Radius * 3.141692;
    }
    else if (type == typeof(SimpleRect))
    {
        SimpleRect rect = (SimpleRect)simpleOb;
        calculatedArea = rect.Height * rect.Width;
    }
    else
    {
        calculatedArea = 0;
